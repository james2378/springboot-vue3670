		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/course_materials/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_course_materials = !show_list_course_materials">
		                        <div class="switch_box">
		                            <span v-if="show_list_course_materials"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--课程资料推荐列表组件 -->
		                <list_course_materials v-if="show_list_course_materials" :list="list_course_materials" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_course_materials.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_course_materials" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_course_materials" :key="i" @click="to_details('course_materials',o,'course_materials_id')">
		                            <td v-for="(oj,n) in fields_course_materials">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_course_materials: true,
				list_course_materials: [],
				vm_arr_course_materials: [
					"course_materials_id",
					"course_number",
					"course_name",
					"teacher",
		...
		methods: {
			...
			get_course_materials() {
				let url = "~/api/course_materials/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 4
				}, (json) => {
					if (json.result) {
						this.list_course_materials = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_course_materials() {
		        var list = this.list_course_materials;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_course_materials.map((o) => {
		                // 第二个中括号是对象的属性
