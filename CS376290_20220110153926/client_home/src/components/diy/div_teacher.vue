<template>
	<div class="diy_details_box diy_div_teacher">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "teacher_id",
					source_id: this.obj["teacher_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/teacher/set?teacher_id=' + _this.obj["teacher_id"], {
								praise_len
							}, (res) => {
								if(res.result){
