<template>
	<div class="diy_details_box diy_div_learning_statistics">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "learning_statistics_id",
					source_id: this.obj["learning_statistics_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/learning_statistics/set?learning_statistics_id=' + _this.obj["learning_statistics_id"], {
								praise_len
							}, (res) => {
								if(res.result){
