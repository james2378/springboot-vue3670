<template>
	<div class="diy_details_box diy_div_course_management">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "course_management_id",
					source_id: this.obj["course_management_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/course_management/set?course_management_id=' + _this.obj["course_management_id"], {
								praise_len
							}, (res) => {
								if(res.result){
